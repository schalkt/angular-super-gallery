...
export class ServiceController {
	...
	public setOptions(options: IOptions) {
		...
		if (options) {
			...
			options = merge(copy(this.defaults), options);
			...
			this.options = copy(this.defaults);
		}
	...
	public setFocus() {
		...
		if (this.modalVisible) {
			...
			const el: any = this.el('div.asg-modal.' + this.id + ' .keyInput')[0];
	...
	public editGallery(edit: IEdit, component) {
		...
		if (edit.add) {
			...
			const editLength = edit.add.length;
			for (let editKey = 0; editKey < editLength; editKey++) {
		...
		if (edit.update) {
			...
			const length = edit.update.length;
			for (let key = 0; key < length; key++) {
				this.addImage(edit.update[key], key);
			}

			const count = this.files.length - edit.update.length;
			if (count > 0) {
...
export const asgService = ['$timeout', '$interval', '$location', '$rootScope', '$window', ServiceController];
